...
const statisticalStatus = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    try {
      fetch(API_URL)
        .then(response => response.json())
        .then(data => {
          const mapping_data = data.filter(
            item =>
              moment().add(-7, 'd').format('YYYY/MM/DD') <= item.enter_time &&
              item.enter_time <= moment().format('YYYY/MM/DD'),
          );
          ...
          mapping_data.map(user => {
            ...
            if (
              ...
              temp_arr[
                parseInt(user.enter_time.substring(11, 13), 10) - 10
              ] += 1;
            }
          });
  ...
  return (
    <View
      style={{justifyContent: 'center', flexDirection: 'column', margin: 10}}>
      <View>
